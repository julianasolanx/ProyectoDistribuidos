...
def main():
    ...
    try:
        while True:
            ...
            tiempo_permanencia_seg = cmd.get("tiempo_permanencia_seg", 15)
            ...
            if act_calle != ant_calle or act_carrera != ant_carrera:
                ...
                log(
                    f"    🚦 Semáforo CALLE (Fila) : [{ant_calle}] ➡️ [{act_calle}] por {tiempo_permanencia_seg}s"
                )
                log(
                    f"    🚦 Semáforo CARRERA (Columna): [{ant_carrera}] ➡️ [{act_carrera}] por {tiempo_permanencia_seg}s"
                )
                log(f"    ℹ️ Motivo: {motivo}")
            else:
                log(
                    f"⏸️ [SOSTIENE] Intersección {interseccion} sin variaciones -> CALLE: [{act_calle}] | CARRERA: [{act_carrera}] (Permanencia restante estimada: {tiempo_permanencia_seg}s)"
                )

...
class ValidadorSeguridad:
    ...
    def validar_payload_sensor(topico, datos):
        ...
        try:
            if not isinstance(datos["interseccion"], str) or not datos[
                "interseccion"
            ].startswith("INT_"):
                return (
                    False,
                    "Formato de identificador de intersección inválido o malicioso.",
                )

            if topico == "camara":
                if not isinstance(datos["volumen"], (int, float)) or not isinstance(
                    datos["velocidad_promedio"], (int, float)
                ):
...
class MotorCorrelacion:
    ...
    def actualizar_y_evaluar(self, topico, datos):
        ...
        if interseccion not in self.bufer_intersecciones:
            self.bufer_intersecciones[interseccion] = {
                "camara": None,
                "gps": None,
                "espira_inductiva": None,
            }
        ...
        if not camara or not gps or not espira:
            return (
                "ESPERAR",
                "Ventana de correlación en proceso de sincronización...",
                15,
            )
        ...
        if q_cola < 5 and v_promedio_real > 35 and nivel_g == "BAJA" and v_conteo <= 5:
            return (
                "cambiar_rojo",
                "REGLA_1 (Normalidad): Flujo despejado en el eje de la calle.",
                15,
            )
        elif (
            q_cola >= 13
            and v_promedio_real < 15
            and nivel_g == "ALTA"
            and v_conteo >= 16
        ):
            return (
                "extender_verde",
                "REGLA_2 (Congestión Crítica): Forzando Ola Verde.",
                30,
            )
        elif 5 <= q_cola < 13 or (15 <= v_promedio_real <= 35) or nivel_g == "NORMAL":
            return (
                "cambiar_verde",
                "REGLA_3 (Congestión Moderada): Agilizando paso.",
                20,
            )

        return "mantener", "Malla vial estable dentro de rangos.", 15
...
def main():
    if len(sys.argv) != 11:
        ...
        print("Uso correcto:")
        print(
            "  python servicio_analitica.py <ip_escucha_rep> <ip_broker> <port_sub> <ip_control> <port_push> \\"
        )
        print(
            "                               <ip_bd_p> <port_bd_p> <ip_bd_r> <port_bd_r> <port_rep_monitoreo>"
        )
        sys.exit(1)
    ...
    log(
        f"Servicio de Analítica Hardened activo. Interfaz de control acoplada a: {IP_LOCAL_REP}:{PORT_REP}"
    )
    ...
    try:
        while True:
            ...
            if socket_sub in socks:
                ...
                if len(partes) != 2:
                    log(
                        "[ALERTA SEGURIDAD] Mensaje recibido sin delimitador de tópico estructural."
                    )
                    continue
                ...
                try:
                    ...
                    log(
                        "[ALERTA SEGURIDAD] Intento de desbordamiento: El payload no es un JSON parseable."
                    )
                    continue

                es_valido, motivo_seguridad = ValidadorSeguridad.validar_payload_sensor(
                    topico, datos
                )
                if not es_valido:
                    log(
                        f"🚨 [MENSAJE RECHAZADO] Violación de integridad detectada: {motivo_seguridad}"
                    )
                    continue
                ...
                accion, motivo, tiempo = motor.actualizar_y_evaluar(topico, datos)
                ...
                if accion in ["cambiar_verde", "cambiar_rojo", "extender_verde"]:
                    cmd = {
                        ...
                        "accion": accion,
                        "motivo": motivo,
                        "tiempo_permanencia_seg": tiempo,
                    }
                    socket_control.send_string(json.dumps(cmd))

                estado_calle = (
                    "VERDE"
                    if accion in ["cambiar_verde", "extender_verde", "mantener"]
                    else "ROJO"
                )
                ...
                try:
                    socket_bd_r.send_string(msg_send, zmq.NOBLOCK)
                except zmq.Again:
                    pass
            ...
            if socket_rep_monitoreo in socks:
                ...
                try:
                    ...
                    socket_rep_monitoreo.send_string(
                        json.dumps(
                            {"status": "REJECTED", "mensaje": "Payload corrupto"}
                        )
                    )
                    continue
                ...
                if not isinstance(interseccion_m, str) or len(interseccion_m) > 10:
                    socket_rep_monitoreo.send_string(
                        json.dumps(
                            {
                                "status": "REJECTED",
                                "mensaje": "Inyección string inválida",
                            }
                        )
                    )
                    continue
                ...
                cmd_directo = {
                    "interseccion": interseccion_m,
                    "accion": "extender_verde",
                    "motivo": cmd_manual.get("motivo"),
